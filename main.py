...
def abrir_sped():
    arquivo = filedialog.askopenfilename(filetypes=[("Arquivo SPED","*.txt")])
    if not arquivo:
        return
    try:
        ...
        if reg:
            ...
            lbl_periodo.config(text=f"Período: {formatar_data(reg[4])} até {formatar_data(reg[5])}")
        ...
        if pasta_xml and os.path.isdir(pasta_xml):
            card_xml.atualizar(len([f for f in os.listdir(pasta_xml) if f.lower().endswith(".xml")]))

        texto=FormatadorAuditoria.montar_resumo(lbl_empresa.cget("text").replace("Empresa: ",""),
...
topo.pack(fill=X)
Label(topo,text="FiscalPro",bg="#0b5ed7",fg="white",font=("Segoe UI",22,"bold")).pack(pady=(10,0))
Label(topo,text="Assistente Fiscal Inteligente",bg="#0b5ed7",fg="white").pack()

frame_dashboard=ttk.Frame(janela)
frame_dashboard.pack(fill=X,padx=15,pady=10)
card_registros=Card(frame_dashboard,"Registros"); card_registros.grid(row=0,column=0,padx=8)
card_erros=Card(frame_dashboard,"Erros"); card_erros.grid(row=0,column=1,padx=8)
card_avisos=Card(frame_dashboard,"Avisos"); card_avisos.grid(row=0,column=2,padx=8)
card_xml=Card(frame_dashboard,"XML"); card_xml.grid(row=0,column=3,padx=8)

frame_info=ttk.LabelFrame(janela,text="Informações")
frame_info.pack(fill=X,padx=15,pady=10)
lbl_empresa=ttk.Label(frame_info,text="Empresa: -"); lbl_empresa.grid(row=0,column=0,sticky="w",padx=10,pady=5)
lbl_periodo=ttk.Label(frame_info,text="Período: -"); lbl_periodo.grid(row=1,column=0,sticky="w",padx=10,pady=5)
lbl_sped=ttk.Label(frame_info,text="SPED: -"); lbl_sped.grid(row=2,column=0,sticky="w",padx=10,pady=5)
lbl_xml=ttk.Label(frame_info,text="XML: -"); lbl_xml.grid(row=3,column=0,sticky="w",padx=10,pady=5)

fb=ttk.Frame(janela); fb.pack()
ttk.Button(fb,text="Abrir SPED",command=abrir_sped).grid(row=0,column=0,padx=5)
ttk.Button(fb,text="Selecionar XML",command=selecionar_pasta_xml).grid(row=0,column=1,padx=5)

fr=ttk.LabelFrame(janela,text="Resultado")
fr.pack(fill=BOTH,expand=True,padx=15,pady=10)
txt_resultado=Text(fr,wrap=WORD)
txt_resultado.pack(fill=BOTH,expand=True)

barra=Frame(janela,bg="#e9ecef")
barra.pack(fill=X,side=BOTTOM)
lbl_status=Label(barra,text="Pronto.",bg="#e9ecef")
